...
const TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description:
      'Get the accessibility tree of the current page. Returns an interactive tree with element refs (@e1, @e2, etc.) for deterministic interaction.',
    inputSchema: {
      type: 'object',
      properties: {
        interactiveOnly: {
...
async function handleToolCall(name: string, args: any): Promise<any> {
  ...
  switch (name) {
    ...
    case 'browser_ios_device_list':
      return execute({ id: generateId(), action: 'device_list' } as DeviceListCommand);
    ...
    case 'browser_back':
      return execute({ id: generateId(), action: 'back' } as BackCommand);

    case 'browser_forward':
      return execute({ id: generateId(), action: 'forward' } as ForwardCommand);

    case 'browser_reload':
      return execute({ id: generateId(), action: 'reload' } as ReloadCommand);

    case 'browser_url':
      return execute({ id: generateId(), action: 'url' } as UrlCommand);

    case 'browser_title':
      return execute({ id: generateId(), action: 'title' } as TitleCommand);
    ...
    case 'browser_tab_list':
      return execute({ id: generateId(), action: 'tab_list' } as TabListCommand);
    ...
    case 'browser_main_frame':
      return execute({ id: generateId(), action: 'mainframe' } as MainFrameCommand);
    ...
    case 'browser_add_init_script':
      return execute({
        ...
        script: args.script,
      } as AddInitScriptCommand);

    case 'browser_bring_to_front':
      return execute({
        id: generateId(),
        action: 'bringtofront',
      } as BringToFrontCommand);
    ...
    case 'browser_recording_start':
      return execute({
        ...
        path: args.path,
        url: args.url,
      } as RecordingStartCommand);

    case 'browser_recording_stop':
      return execute({
        id: generateId(),
        action: 'recording_stop',
      } as RecordingStopCommand);
